...
4. Open the **"Content Performance"** database in Notion → "..." menu →
   Connections → add this integration. Repeat for the **"Meta Ads"** and
   **"Weekly Update Draft"** databases (the last one is owned by the
   Agency Command Center project, but the "PM Review" tab needs read/write
   access to it too).
...
- The backend is otherwise read-only. The one exception is the "PM Review"
  tab's `doPost` (action `saveDraftReview`), which writes exactly 4 fields
  (`PM Working Notes`, `Next Week Plan`, `Next Week Focus`, `Ask from
  Client`) plus `Status` → `Approved` on the "Weekly Update Draft" data
  source. It never touches `Draft Text`, `Final Text`, or `PM Prompt` —
  those are generated by the Agency Command Center's own Make scenario and
  are treated as read-only from here, to avoid the two systems fighting
